...
(function (LIB) {
    ...
    var ShadowLight = /** @class */ (function (_super) {
        ...
        ShadowLight.prototype.computeTransformedInformation = function () {
            if (this.parent && this.parent.getWorldMatrix) {
                if (!this.transformedPosition) {
                    this.transformedPosition = LIB.Vector3.Zero();
                }
                LIB.Vector3.TransformCoordinatesToRef(this.position, this.parent.getWorldMatrix(), this.transformedPosition);
                // In case the direction is present.
                if (this.direction) {
                    if (!this.transformedDirection) {
                        this.transformedDirection = LIB.Vector3.Zero();
                    }
                    LIB.Vector3.TransformNormalToRef(this.direction, this.parent.getWorldMatrix(), this.transformedDirection);
                }
                return true;
            }
            return false;
        ...
        ShadowLight.prototype.getRotation = function () {
            ...
            var xaxis = LIB.Vector3.Cross(this.direction, LIB.Axis.Y);
            var yaxis = LIB.Vector3.Cross(xaxis, this.direction);
        ...
        ShadowLight.prototype._getWorldMatrix = function () {
            ...
            LIB.Matrix.TranslationToRef(this.position.x, this.position.y, this.position.z, this._worldMatrix);
