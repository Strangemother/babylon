    ...
    var SpriteManager = /** @class */ (function () {
        function SpriteManager(name, imgUrl, capacity, cellSize, scene, epsilon, samplingMode) {
            ...
            */
            ...
            this._spriteTexture = new LIB.Texture(imgUrl, scene, true, false, samplingMode);
            this._spriteTexture.wrapU = LIB.Texture.CLAMP_ADDRESSMODE;
            this._spriteTexture.wrapV = LIB.Texture.CLAMP_ADDRESSMODE;
            ...
            this._buffer = new LIB.Buffer(scene.getEngine(), this._vertexData, true, 16);
            var positions = this._buffer.createVertexBuffer(LIB.VertexBuffer.PositionKind, 0, 4);
            var options = this._buffer.createVertexBuffer("options", 4, 4);
            var cellInfo = this._buffer.createVertexBuffer("cellInfo", 8, 4);
            var colors = this._buffer.createVertexBuffer(LIB.VertexBuffer.ColorKind, 12, 4);
            ...
            // Effects
            this._effectBase = this._scene.getEngine().createEffect("sprites", [LIB.VertexBuffer.PositionKind, "options", "cellInfo", LIB.VertexBuffer.ColorKind], ["view", "projection", "textureInfos", "alphaTest"], ["diffuseSampler"], "");
            this._effectFog = this._scene.getEngine().createEffect("sprites", [LIB.VertexBuffer.PositionKind, "options", "cellInfo", LIB.VertexBuffer.ColorKind], ["view", "projection", "textureInfos", "alphaTest", "vFogInfos", "vFogColor"], ["diffuseSampler"], "#define FOG");
        }
        ...
        SpriteManager.prototype.intersects = function (ray, camera, predicate, fastCheck) {
            ...
            var min = LIB.Vector3.Zero();
            var max = LIB.Vector3.Zero();
            var distance = Number.MAX_VALUE;
            var currentSprite = null;
            var cameraSpacePosition = LIB.Vector3.Zero();
            ...
            for (var index = 0; index < count; index++) {
                ...
                LIB.Vector3.TransformCoordinatesToRef(sprite.position, cameraView, cameraSpacePosition);
                min.copyFromFloats(cameraSpacePosition.x - sprite.width / 2, cameraSpacePosition.y - sprite.height / 2, cameraSpacePosition.z);
                max.copyFromFloats(cameraSpacePosition.x + sprite.width / 2, cameraSpacePosition.y + sprite.height / 2, cameraSpacePosition.z);
                if (ray.intersectsBoxMinMax(min, max)) {
                    var currentDistance = LIB.Vector3.Distance(cameraSpacePosition, ray.origin);
            ...
            if (currentSprite) {
                var result = new LIB.PickingInfo();
                result.hit = true;
        ...
        SpriteManager.prototype.render = function () {
            ...
            if (this._scene.fogEnabled && this._scene.fogMode !== LIB.Scene.FOGMODE_NONE && this.fogEnabled) {
            ...
            engine.drawElementsType(LIB.Material.TriangleFillMode, 0, max * 6);
