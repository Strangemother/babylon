    ...
    var SolidParticleSystem = /** @class */ (function () {
        ...
        */
        function SolidParticleSystem(name, scene, options) {
            ...
            */
            ...
            this._copy = new LIB.SolidParticle(0, 0, 0, null, 0, 0, this);
            this._color = new LIB.Color4(0, 0, 0, 0);
            ...
            this._cam_axisZ = LIB.Vector3.Zero();
            this._cam_axisY = LIB.Vector3.Zero();
            this._cam_axisX = LIB.Vector3.Zero();
            this._axisZ = LIB.Axis.Z;
            this._camDir = LIB.Vector3.Zero();
            this._camInvertedPosition = LIB.Vector3.Zero();
            this._rotMatrix = new LIB.Matrix();
            this._invertMatrix = new LIB.Matrix();
            this._rotated = LIB.Vector3.Zero();
            this._quaternion = new LIB.Quaternion();
            this._vertex = LIB.Vector3.Zero();
            this._normal = LIB.Vector3.Zero();
            this._yaw = 0.0;
            this._pitch = 0.0;
            this._roll = 0.0;
            this._halfroll = 0.0;
        ...
        */
        SolidParticleSystem.prototype.digest = function (mesh, options) {
            ...
            var meshUV = mesh.getVerticesData(LIB.VertexBuffer.UVKind);
            var meshCol = mesh.getVerticesData(LIB.VertexBuffer.ColorKind);
            var meshNor = mesh.getVerticesData(LIB.VertexBuffer.NormalKind);
            ...
            while (f < totalFacets) {
                ...
                if (this._particlesIntersect) {
                    bInfo = new LIB.BoundingInfo(barycenter, barycenter);
                }
                var modelShape = new LIB.ModelShape(this._shapeCounter, shape, size * 3, shapeUV, null, null);
        ...
        SolidParticleSystem.prototype._unrotateFixedNormals = function () {
            ...
            for (var p = 0; p < this.particles.length; p++) {
                ...
                for (var pt = 0; pt < this._shape.length; pt++) {
                    idx = index + pt * 3;
                    LIB.Vector3.TransformNormalFromFloatsToRef(this._normals32[idx], this._normals32[idx + 1], this._normals32[idx + 2], this._invertMatrix, this._normal);
        ...
        SolidParticleSystem.prototype._meshBuilder = function (p, shape, positions, meshInd, indices, meshUV, uvs, meshCol, colors, meshNor, normals, idx, idxInShape, options) {
            ...
            for (i = 0; i < shape.length; i++) {
                ...
                if (!this.recomputeNormals && meshNor) {
                    ...
                    LIB.Vector3.TransformNormalToRef(this._normal, this._rotMatrix, this._normal);
            ...
            if (this._depthSort) {
                this.depthSortedParticles.push(new LIB.DepthSortedParticle());
            }
            return this._copy;
        };
        ...
        SolidParticleSystem.prototype._posToShape = function (positions) {
            ...
            for (var i = 0; i < positions.length; i += 3) {
                shape.push(new LIB.Vector3(positions[i], positions[i + 1], positions[i + 2]));
            }
            return shape;
        };
        ...
        SolidParticleSystem.prototype._addParticle = function (idx, idxpos, idxind, model, shapeId, idxInShape, bInfo) {
            ...
            var sp = new LIB.SolidParticle(idx, idxpos, idxind, model, shapeId, idxInShape, this, bInfo);
        ...
        */
        SolidParticleSystem.prototype.addShape = function (mesh, nb, options) {
            ...
            var modelShape = new LIB.ModelShape(this._shapeCounter, shape, meshInd.length, shapeUV, posfunc, vtxfunc);
            // particles
            var sp;
            var currentCopy;
            var idx = this.nbParticles;
        ...
        */
        SolidParticleSystem.prototype.rebuildMesh = function () {
            ...
            this.mesh.updateVerticesData(LIB.VertexBuffer.PositionKind, this._positions32, false, false);
            return this;
        };
        /**
        ...
        */
        SolidParticleSystem.prototype.setParticles = function (start, end, update) {
            ...
            if (this.billboard) {
                ...
                LIB.Vector3.TransformNormalToRef(this._camDir, this._invertMatrix, this._cam_axisZ);
                this._cam_axisZ.normalize();
                // same for camera up vector extracted from the cam view matrix
                var view = this._camera.getViewMatrix(true);
                LIB.Vector3.TransformNormalFromFloatsToRef(view.m[1], view.m[5], view.m[9], this._invertMatrix, this._cam_axisY);
                LIB.Vector3.CrossToRef(this._cam_axisY, this._cam_axisZ, this._cam_axisX);
                this._cam_axisY.normalize();
                this._cam_axisX.normalize();
            }
            // if depthSort, compute the camera global position in the mesh local system
            if (this._depthSort) {
                LIB.Vector3.TransformCoordinatesToRef(this._camera.globalPosition, this._invertMatrix, this._camInvertedPosition); // then un-rotate the camera
            }
            LIB.Matrix.IdentityToRef(this._rotMatrix);
            ...
            for (var p = start; p <= end; p++) {
                ...
                if (this._depthSort && this._depthSortParticles) {
                    ...
                    dsp.sqDistance = LIB.Vector3.DistanceSquared(this._particle.position, this._camInvertedPosition);
                }
            ...
            if (update) {
                if (this._computeParticleColor) {
                    this.mesh.updateVerticesData(LIB.VertexBuffer.ColorKind, this._colors32, false, false);
                }
                if (this._computeParticleTexture) {
                    this.mesh.updateVerticesData(LIB.VertexBuffer.UVKind, this._uvs32, false, false);
                }
                this.mesh.updateVerticesData(LIB.VertexBuffer.PositionKind, this._positions32, false, false);
                if (!this.mesh.areNormalsFrozen || this.mesh.isFacetDataEnabled) {
                    ...
                    if (!this.mesh.areNormalsFrozen) {
                        this.mesh.updateVerticesData(LIB.VertexBuffer.NormalKind, this._normals32, false, false);
                    }
                }
                if (this._depthSort && this._depthSortParticles) {
            ...
            if (this._computeBoundingBox) {
                this.mesh._boundingInfo = new LIB.BoundingInfo(this._minimum, this._maximum);
