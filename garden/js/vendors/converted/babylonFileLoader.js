                                            ...
                                            if (found === true || !parsedData.geometries[geometryType] || !(Array.isArray(parsedData.geometries[geometryType]))) {
                                                ...
                                                parsedData.geometries[geometryType].forEach(function (parsedGeometryData) {
                                                    if (parsedGeometryData.id === parsedMesh.geometryId) {
                                                        switch (geometryType) {
                                                            case "boxes":
                                                                LIB.Geometry.Primitives.Box.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "spheres":
                                                                LIB.Geometry.Primitives.Sphere.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "cylinders":
                                                                LIB.Geometry.Primitives.Cylinder.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "toruses":
                                                                LIB.Geometry.Primitives.Torus.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "grounds":
                                                                LIB.Geometry.Primitives.Ground.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "planes":
                                                                LIB.Geometry.Primitives.Plane.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "torusKnots":
                                                                LIB.Geometry.Primitives.TorusKnot.Parse(parsedGeometryData, scene);
                                                                break;
                                                            case "vertexData":
                ...
                try {
                    ...
                    if (geometries !== undefined && geometries !== null) {
                        ...
                        if (boxes !== undefined && boxes !== null) {
                            for (index = 0, cache = boxes.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Box.Parse(parsedBox, scene);
                            }
                        }
                        // Spheres
                        ...
                        if (spheres !== undefined && spheres !== null) {
                            for (index = 0, cache = spheres.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Sphere.Parse(parsedSphere, scene);
                            }
                        }
                        // Cylinders
                        ...
                        if (cylinders !== undefined && cylinders !== null) {
                            for (index = 0, cache = cylinders.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Cylinder.Parse(parsedCylinder, scene);
                            }
                        }
                        // Toruses
                        ...
                        if (toruses !== undefined && toruses !== null) {
                            for (index = 0, cache = toruses.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Torus.Parse(parsedTorus, scene);
                            }
                        }
                        // Grounds
                        ...
                        if (grounds !== undefined && grounds !== null) {
                            for (index = 0, cache = grounds.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Ground.Parse(parsedGround, scene);
                            }
                        }
                        // Planes
                        ...
                        if (planes !== undefined && planes !== null) {
                            for (index = 0, cache = planes.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.Plane.Parse(parsedPlane, scene);
                            }
                        }
                        // TorusKnots
                        ...
                        if (torusKnots !== undefined && torusKnots !== null) {
                            for (index = 0, cache = torusKnots.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Primitives.TorusKnot.Parse(parsedTorusKnot, scene);
                            }
                        }
                        // VertexData
                        ...
                        if (vertexData !== undefined && vertexData !== null) {
                            for (index = 0, cache = vertexData.length; index < cache; index++) {
                                ...
                                LIB.Geometry.Parse(parsedVertexData, scene, rootUrl);
                            }
                        }
                    }
                    // Transform nodes
                    if (parsedData.transformNodes !== undefined && parsedData.transformNodes !== null) {
                        for (index = 0, cache = parsedData.transformNodes.length; index < cache; index++) {
                            ...
                            LIB.TransformNode.Parse(parsedTransformNode, scene, rootUrl);
                        }
                    }
                    // Meshes
                    if (parsedData.meshes !== undefined && parsedData.meshes !== null) {
                        ...
                        for (index = 0, cache = parsedData.sounds.length; index < cache; index++) {
                            ...
                            if (LIB.Engine.audioEngine.canUseWebAudio) {
                                ...
                                if (!loadedSounds[parsedSound.url]) {
                                    ...
                                    LIB.Sound.Parse(parsedSound, scene, rootUrl, loadedSounds[parsedSound.url]);
                                }
                            }
                            else {
                                new LIB.Sound(parsedSound.name, null, scene);
                            }
                        }
                    }
                    loadedSounds = [];
                    ...
                    if (parsedData.particleSystems !== undefined && parsedData.particleSystems !== null) {
                        for (index = 0, cache = parsedData.particleSystems.length; index < cache; index++) {
                            var parsedParticleSystem = parsedData.particleSystems[index];
                            LIB.ParticleSystem.Parse(parsedParticleSystem, scene, rootUrl);
                        }
                    }
                    // Environment texture
                    if (parsedData.environmentTexture !== undefined && parsedData.environmentTexture !== null) {
                        scene.environmentTexture = LIB.CubeTexture.CreateFromPrefilteredData(rootUrl + parsedData.environmentTexture, scene);
                    ...
                    if (parsedData.lensFlareSystems !== undefined && parsedData.lensFlareSystems !== null) {
                        for (index = 0, cache = parsedData.lensFlareSystems.length; index < cache; index++) {
                            ...
                            LIB.LensFlareSystem.Parse(parsedLensFlareSystem, scene, rootUrl);
                        }
                    }
                    // Shadows
                    if (parsedData.shadowGenerators !== undefined && parsedData.shadowGenerators !== null) {
                        for (index = 0, cache = parsedData.shadowGenerators.length; index < cache; index++) {
                            ...
                            LIB.ShadowGenerator.Parse(parsedShadowGenerator, scene);
                        }
                    }
