(function (LIB) {
    ...
    var Vertex = /** @class */ (function () {
        ...
        Vertex.prototype.interpolate = function (other, t) {
            return new Vertex(LIB.Vector3.Lerp(this.pos, other.pos, t), LIB.Vector3.Lerp(this.normal, other.normal, t), LIB.Vector2.Lerp(this.uv, other.uv, t));
        };
        return Vertex;
    }());
    // # class Plane
    ...
    var Plane = /** @class */ (function () {
        ...
        Plane.FromPoints = function (a, b, c) {
            ...
            var n = LIB.Vector3.Normalize(LIB.Vector3.Cross(v0, v1));
            return new Plane(n, LIB.Vector3.Dot(n, a));
        };
        ...
        Plane.prototype.splitPolygon = function (polygon, coplanarFront, coplanarBack, front, back) {
            ...
            for (i = 0; i < polygon.vertices.length; i++) {
                t = LIB.Vector3.Dot(this.normal, polygon.vertices[i].pos) - this.w;
            ...
            switch (polygonType) {
                case COPLANAR:
                    (LIB.Vector3.Dot(this.normal, polygon.plane.normal) > 0 ? coplanarFront : coplanarBack).push(polygon);
                    break;
                case FRONT:
                    front.push(polygon);
                    break;
                ...
                case SPANNING:
                    ...
                    for (i = 0; i < polygon.vertices.length; i++) {
                        ...
                        if ((ti | tj) === SPANNING) {
                            t = (this.w - LIB.Vector3.Dot(this.normal, vi.pos)) / LIB.Vector3.Dot(this.normal, vj.pos.subtract(vi.pos));
        ...
        // `LIB.CSG.Plane.EPSILON` is the tolerance used by `splitPolygon()` to decide if a
    ...
    var CSG = /** @class */ (function () {
        ...
        CSG.FromMesh = function (mesh) {
            ...
            if (mesh instanceof LIB.Mesh) {
                ...
                throw 'LIB.CSG: Wrong Mesh type, must be LIB.Mesh';
            }
            var indices = mesh.getIndices(), positions = mesh.getVerticesData(LIB.VertexBuffer.PositionKind), normals = mesh.getVerticesData(LIB.VertexBuffer.NormalKind), uvs = mesh.getVerticesData(LIB.VertexBuffer.UVKind);
            ...
            for (var sm = 0, sml = subMeshes.length; sm < sml; sm++) {
                for (var i = subMeshes[sm].indexStart, il = subMeshes[sm].indexCount + subMeshes[sm].indexStart; i < il; i += 3) {
                    vertices = [];
                    for (var j = 0; j < 3; j++) {
                        var sourceNormal = new LIB.Vector3(normals[indices[i + j] * 3], normals[indices[i + j] * 3 + 1], normals[indices[i + j] * 3 + 2]);
                        uv = new LIB.Vector2(uvs[indices[i + j] * 2], uvs[indices[i + j] * 2 + 1]);
                        var sourcePosition = new LIB.Vector3(positions[indices[i + j] * 3], positions[indices[i + j] * 3 + 1], positions[indices[i + j] * 3 + 2]);
                        position = LIB.Vector3.TransformCoordinates(sourcePosition, matrix);
                        normal = LIB.Vector3.TransformNormal(sourceNormal, matrix);
        ...
        // Construct a LIB.CSG solid from a list of `LIB.CSG.Polygon` instances.
        ...
        // Return a new LIB.CSG solid with solid and empty space switched. This solid is
        // not modified.
        ...
        CSG.prototype.buildMeshGeometry = function (name, scene, keepSubMeshes) {
            ...
            matrix.invert();
            var mesh = new LIB.Mesh(name, scene), vertices = [], indices = [], normals = [], uvs = [], vertex = LIB.Vector3.Zero(), normal = LIB.Vector3.Zero(), uv = LIB.Vector2.Zero(), polygons = this.polygons, polygonIndices = [0, 0, 0], polygon, vertice_dict = {}, vertex_idx, currentIndex = 0, subMesh_dict = {}, subMesh_obj;
            if (keepSubMeshes) {
            ...
            for (var i = 0, il = polygons.length; i < il; i++) {
                ...
                for (var j = 2, jl = polygon.vertices.length; j < jl; j++) {
                    ...
                    for (var k = 0; k < 3; k++) {
                        ...
                        var localVertex = LIB.Vector3.TransformCoordinates(vertex, matrix);
                        var localNormal = LIB.Vector3.TransformNormal(normal, matrix);
            ...
            mesh.setVerticesData(LIB.VertexBuffer.PositionKind, vertices);
            mesh.setVerticesData(LIB.VertexBuffer.NormalKind, normals);
            mesh.setVerticesData(LIB.VertexBuffer.UVKind, uvs);
            ...
            if (keepSubMeshes) {
                ...
                for (var m in subMesh_dict) {
                    ...
                    for (var sm in subMesh_dict[m]) {
                        ...
                        LIB.SubMesh.CreateFromIndices(subMesh_obj.materialIndex + materialIndexOffset, subMesh_obj.indexStart, subMesh_obj.indexEnd - subMesh_obj.indexStart + 1, mesh);
