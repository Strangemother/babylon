(function (LIB) {
    ...
    var finalizeSingleMesh = function (mesh, serializationObject) {
        ...
        if (mesh.delayLoadState === LIB.Engine.DELAYLOADSTATE_LOADED || mesh.delayLoadState === LIB.Engine.DELAYLOADSTATE_NONE) {
            //serialize material
            if (mesh.material) {
                if (mesh.material instanceof LIB.StandardMaterial) {
                ...
                else if (mesh.material instanceof LIB.MultiMaterial) {
    ...
    var SceneSerializer = /** @class */ (function () {
        ...
        SceneSerializer.Serialize = function (scene) {
            ...
            // Animations
            LIB.Animation.AppendSerializedAnimations(scene, serializationObject);
            // Materials
                ...
                for (var i = 0; i < toSerialize.length; ++i) {
                    if (withChildren) {
                        toSerialize[i].getDescendants().forEach(function (node) {
                            if (node instanceof LIB.Mesh && (toSerialize.indexOf(node) < 0)) {
