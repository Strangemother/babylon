(function (LIB) {
    ...
    LIB.SimplificationSettings = SimplificationSettings;
    var SimplificationQueue = /** @class */ (function () {
        ...
        SimplificationQueue.prototype.runSimplification = function (task) {
            ...
            if (task.parallelProcessing) {
                ...
                LIB.AsyncLoop.Run(task.settings.length, function (loop) {
    ...
    LIB.DecimationTriangle = DecimationTriangle;
    ...
    LIB.DecimationVertex = DecimationVertex;
    ...
    LIB.QuadraticMatrix = QuadraticMatrix;
     ...
     * Ported mostly from QSlim and http://voxels.blogspot.de/2014/05/quadric-mesh-simplification-with-source.html to LIB JS
     * @author RaananW
     */
    var QuadraticErrorSimplification = /** @class */ (function () {
        ...
        QuadraticErrorSimplification.prototype.runDecimation = function (settings, submeshIndex, successCallback) {
            ...
            var iterationFunction = function (iteration, callback) {
                setTimeout(function () {
                        ...
                        for (var j = 0; j < 3; ++j) {
                            if (t.error[j] < threshold) {
                                ...
                                if (v0.isBorder || v1.isBorder)
                                    continue;
                                var p = LIB.Vector3.Zero();
                                var n = LIB.Vector3.Zero();
                                var uv = LIB.Vector2.Zero();
                                var color = new LIB.Color4(0, 0, 0, 1);
                    ...
                    LIB.AsyncLoop.SyncAsyncForLoop(_this.triangles.length, _this.syncIterations, trianglesIterator, callback, function () { return (triangleCount - deletedTriangles <= targetCount); });
                }, 0);
            };
            LIB.AsyncLoop.Run(this.decimationIterations, function (loop) {
        ...
        QuadraticErrorSimplification.prototype.initWithMesh = function (submeshIndex, callback, optimizeMesh) {
            ...
            this.vertices = [];
            this.triangles = [];
            var positionData = this._mesh.getVerticesData(LIB.VertexBuffer.PositionKind);
            ...
            var vertexInit = function (i) {
                ...
                var position = LIB.Vector3.FromArray(positionData, offset * 3);
            ...
            LIB.AsyncLoop.SyncAsyncForLoop(totalVertices, (this.syncIterations / 4) >> 0, vertexInit, function () {
                ...
                LIB.AsyncLoop.SyncAsyncForLoop(submesh.indexCount / 3, _this.syncIterations, indicesInit, function () {
                    _this.init(callback);
                });
            });
        };
        QuadraticErrorSimplification.prototype.init = function (callback) {
            ...
            var triangleInit1 = function (i) {
                ...
                t.normal = LIB.Vector3.Cross(t.vertices[1].position.subtract(t.vertices[0].position), t.vertices[2].position.subtract(t.vertices[0].position)).normalize();
                for (var j = 0; j < 3; j++) {
                    t.vertices[j].q.addArrayInPlace(QuadraticMatrix.DataFromNumbers(t.normal.x, t.normal.y, t.normal.z, -(LIB.Vector3.Dot(t.normal, t.vertices[0].position))));
                }
            };
            LIB.AsyncLoop.SyncAsyncForLoop(this.triangles.length, this.syncIterations, triangleInit1, function () {
                ...
                LIB.AsyncLoop.SyncAsyncForLoop(_this.triangles.length, _this.syncIterations, triangleInit2, function () {
                    callback();
                });
            });
        };
        QuadraticErrorSimplification.prototype.reconstructMesh = function (submeshIndex) {
            ...
            var newPositionData = (this._reconstructedMesh.getVerticesData(LIB.VertexBuffer.PositionKind) || []);
            var newNormalData = (this._reconstructedMesh.getVerticesData(LIB.VertexBuffer.NormalKind) || []);
            var newUVsData = (this._reconstructedMesh.getVerticesData(LIB.VertexBuffer.UVKind) || []);
            var newColorsData = (this._reconstructedMesh.getVerticesData(LIB.VertexBuffer.ColorKind) || []);
            var normalData = this._mesh.getVerticesData(LIB.VertexBuffer.NormalKind);
            var uvs = this._mesh.getVerticesData(LIB.VertexBuffer.UVKind);
            var colorsData = this._mesh.getVerticesData(LIB.VertexBuffer.ColorKind);
            var vertexCount = 0;
            for (i = 0; i < this.vertices.length; ++i) {
            ...
            this._reconstructedMesh.setVerticesData(LIB.VertexBuffer.PositionKind, newPositionData);
            this._reconstructedMesh.setVerticesData(LIB.VertexBuffer.NormalKind, newNormalData);
            if (newUVsData.length > 0)
                this._reconstructedMesh.setVerticesData(LIB.VertexBuffer.UVKind, newUVsData);
            if (newColorsData.length > 0)
                this._reconstructedMesh.setVerticesData(LIB.VertexBuffer.ColorKind, newColorsData);
            //create submesh
            var originalSubmesh = this._mesh.subMeshes[submeshIndex];
            if (submeshIndex > 0) {
                this._reconstructedMesh.subMeshes = [];
                submeshesArray.forEach(function (submesh) {
                    LIB.SubMesh.AddToMesh(submesh.materialIndex, submesh.verticesStart, submesh.verticesCount, /* 0, newPositionData.length/3, */ submesh.indexStart, submesh.indexCount, submesh.getMesh());
                });
                LIB.SubMesh.AddToMesh(originalSubmesh.materialIndex, startingVertex, vertexCount, /* 0, newPositionData.length / 3, */ startingIndex, newTriangles.length * 3, this._reconstructedMesh);
            }
        };
        QuadraticErrorSimplification.prototype.initDecimatedMesh = function () {
            this._reconstructedMesh = new LIB.Mesh(this._mesh.name + "Decimated", this._mesh.getScene());
        ...
        QuadraticErrorSimplification.prototype.isFlipped = function (vertex1, vertex2, point, deletedArray, borderFactor, delTr) {
            for (var i = 0; i < vertex1.triangleCount; ++i) {
                ...
                d2 = d2.normalize();
                if (Math.abs(LIB.Vector3.Dot(d1, d2)) > 0.999)
                    return true;
                var normal = LIB.Vector3.Cross(d1, d2).normalize();
                deletedArray[i] = false;
                if (LIB.Vector3.Dot(normal, t.normal) < 0.2)
                    return true;
            }
            return false;
        };
        ...
        QuadraticErrorSimplification.prototype.calculateError = function (vertex1, vertex2, pointResult, normalResult, uvResult, colorResult) {
            ...
            if (qDet !== 0 && !border) {
                ...
                var p3 = (vertex1.position.add(vertex2.position)).divide(new LIB.Vector3(2, 2, 2));
