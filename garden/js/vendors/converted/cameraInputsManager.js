    ...
    var CameraInputsManager = /** @class */ (function () {
        ...
        CameraInputsManager.prototype.add = function (input) {
            ...
            if (this.attached[type]) {
                LIB.Tools.Warn("camera input of type " + type + " already exists on camera");
                return;
            }
        ...
        CameraInputsManager.prototype.serialize = function (serializedCamera) {
            ...
            for (var cam in this.attached) {
                var input = this.attached[cam];
                var res = LIB.SerializationHelper.Serialize(input);
        ...
        CameraInputsManager.prototype.parse = function (parsedCamera) {
            ...
            if (parsedInputs) {
                ...
                for (var n in parsedInputs) {
                    var construct = LIB.CameraInputTypes[n];
                    if (construct) {
                        var parsedinput = parsedInputs[n];
                        var input = LIB.SerializationHelper.Parse(function () { return new construct(); }, parsedinput, null);
                        this.add(input);
                    }
                }
            }
                ...
                for (var n in this.attached) {
                    var construct = LIB.CameraInputTypes[this.attached[n].getClassName()];
                    if (construct) {
                        var input = LIB.SerializationHelper.Parse(function () { return new construct(); }, parsedCamera, null);
