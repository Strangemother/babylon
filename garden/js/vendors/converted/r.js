(function (LIB) {
    var GroundMesh = /** @class */ (function (_super) {
        ...
        GroundMesh.prototype.getHeightAtCoordinates = function (x, z) {
            ...
            var tmpVect = LIB.Tmp.Vector3[8];
            LIB.Vector3.TransformCoordinatesFromFloatsToRef(x, 0.0, z, invMat, tmpVect); // transform x,z in the mesh local space
            x = tmpVect.x;
            z = tmpVect.z;
            ...
            LIB.Vector3.TransformCoordinatesFromFloatsToRef(0.0, y, 0.0, world, tmpVect);
            return tmpVect.y;
        };
        /**
        ...
        GroundMesh.prototype.getNormalAtCoordinates = function (x, z) {
            var normal = new LIB.Vector3(0.0, 1.0, 0.0);
        ...
        GroundMesh.prototype.getNormalAtCoordinatesToRef = function (x, z, ref) {
            var world = this.getWorldMatrix();
            var tmpMat = LIB.Tmp.Matrix[5];
            world.invertToRef(tmpMat);
            var tmpVect = LIB.Tmp.Vector3[8];
            LIB.Vector3.TransformCoordinatesFromFloatsToRef(x, 0.0, z, tmpMat, tmpVect); // transform x,z in the mesh local space
            x = tmpVect.x;
            z = tmpVect.z;
            if (x < this._minX || x > this._maxX || z < this._minZ || z > this._maxZ) {
                return this;
            ...
            var facet = this._getFacetAt(x, z);
            LIB.Vector3.TransformNormalFromFloatsToRef(facet.x, facet.y, facet.z, world, ref);
            return this;
        };
        /**
        ...
        */
        ...
        GroundMesh.prototype._initHeightQuads = function () {
            ...
            for (var row = 0; row < subdivisionsY; row++) {
                for (var col = 0; col < subdivisionsX; col++) {
                    var quad = { slope: LIB.Vector2.Zero(), facet1: new LIB.Vector4(0.0, 0.0, 0.0, 0.0), facet2: new LIB.Vector4(0.0, 0.0, 0.0, 0.0) };
        ...
        GroundMesh.prototype._computeHeightQuads = function () {
            ...
            var v1 = LIB.Tmp.Vector3[3];
            var v2 = LIB.Tmp.Vector3[2];
            var v3 = LIB.Tmp.Vector3[1];
            var v4 = LIB.Tmp.Vector3[0];
            var v1v2 = LIB.Tmp.Vector3[4];
            var v1v3 = LIB.Tmp.Vector3[5];
            var v1v4 = LIB.Tmp.Vector3[6];
            var norm1 = LIB.Tmp.Vector3[7];
            var norm2 = LIB.Tmp.Vector3[8];
            var i = 0;
            var j = 0;
            var k = 0;
            ...
            for (var row = 0; row < subdivisionsY; row++) {
                for (var col = 0; col < subdivisionsX; col++) {
                    ...
                    LIB.Vector3.CrossToRef(v1v4, v1v3, norm1); // caution : CrossToRef uses the Tmp class
                    LIB.Vector3.CrossToRef(v1v2, v1v4, norm2);
                    norm1.normalize();
                    norm2.normalize();
