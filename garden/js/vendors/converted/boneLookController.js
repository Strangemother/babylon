(function (LIB) {
    var BoneLookController = /** @class */ (function () {
        ...
        function BoneLookController(mesh, bone, target, options) {
            ...
            this._fowardAxis = LIB.Vector3.Forward();
            this.mesh = mesh;
            this.bone = bone;
            this.target = target;
            if (options) {
                ...
                if (options.yawAxis != null || options.pitchAxis != null) {
                    var newYawAxis = LIB.Axis.Y;
                    var newPitchAxis = LIB.Axis.X;
                    ...
                    var newRollAxis = LIB.Vector3.Cross(newPitchAxis, newYawAxis);
                    this._transformYawPitch = LIB.Matrix.Identity();
                    LIB.Matrix.FromXYZAxesToRef(newPitchAxis, newYawAxis, newRollAxis, this._transformYawPitch);
                    this._transformYawPitchInv = this._transformYawPitch.clone();
                    this._transformYawPitch.invert();
                }
            }
            if (!bone.getParent() && this.upAxisSpace == LIB.Space.BONE) {
                this.upAxisSpace = LIB.Space.LOCAL;
            }
        }
        ...
        BoneLookController.prototype.update = function () {
            ...
            if (this.upAxisSpace == LIB.Space.BONE && parentBone) {
                if (this._transformYawPitch) {
                    LIB.Vector3.TransformCoordinatesToRef(upAxis, this._transformYawPitchInv, upAxis);
                }
                parentBone.getDirectionToRef(upAxis, this.mesh, upAxis);
            }
            else if (this.upAxisSpace == LIB.Space.LOCAL) {
            ...
            if (checkYaw || checkPitch) {
                ...
                if (this.upAxisSpace == LIB.Space.BONE && upAxis.y == 1 && parentBone) {
                    parentBone.getRotationMatrixToRef(LIB.Space.WORLD, this.mesh, spaceMat);
                }
                else if (this.upAxisSpace == LIB.Space.LOCAL && upAxis.y == 1 && !parentBone) {
                    ...
                    if (this._transformYawPitch) {
                        LIB.Vector3.TransformCoordinatesToRef(forwardAxis, this._transformYawPitchInv, forwardAxis);
                    }
                    if (parentBone) {
                    ...
                    var rightAxis = LIB.Vector3.Cross(upAxis, forwardAxis);
                    rightAxis.normalize();
                    var forwardAxis = LIB.Vector3.Cross(rightAxis, upAxis);
                    LIB.Matrix.FromXYZAxesToRef(rightAxis, upAxis, forwardAxis, spaceMat);
                }
                ...
                if (checkPitch) {
                    ...
                    LIB.Vector3.TransformCoordinatesToRef(localTarget, spaceMatInv, localTarget);
                    ...
                    if (pitch != newPitch) {
                        LIB.Vector3.TransformCoordinatesToRef(localTarget, spaceMat, localTarget);
                ...
                if (checkYaw) {
                    ...
                    if (this._slerping && this._yawRange > Math.PI) {
                        ...
                        boneFwd.copyFrom(LIB.Axis.Z);
                        if (this._transformYawPitch) {
                            LIB.Vector3.TransformCoordinatesToRef(boneFwd, this._transformYawPitchInv, boneFwd);
                        }
                        var boneRotMat = BoneLookController._tmpMats[4];
                        this._boneQuat.toRotationMatrix(boneRotMat);
                        this.mesh.getWorldMatrix().multiplyToRef(boneRotMat, boneRotMat);
                        LIB.Vector3.TransformCoordinatesToRef(boneFwd, boneRotMat, boneFwd);
                        LIB.Vector3.TransformCoordinatesToRef(boneFwd, spaceMatInv, boneFwd);
            ...
            zaxis.normalize();
            LIB.Vector3.CrossToRef(upAxis, zaxis, xaxis);
            xaxis.normalize();
            LIB.Vector3.CrossToRef(zaxis, xaxis, yaxis);
            yaxis.normalize();
            LIB.Matrix.FromXYZAxesToRef(xaxis, yaxis, zaxis, _tmpMat1);
            ...
            if (this.adjustYaw || this.adjustPitch || this.adjustRoll) {
                LIB.Matrix.RotationYawPitchRollToRef(this.adjustYaw, this.adjustPitch, this.adjustRoll, _tmpMat2);
                _tmpMat2.multiplyToRef(_tmpMat1, _tmpMat1);
            }
            if (this.slerpAmount < 1) {
                if (!this._slerping) {
                    this.bone.getRotationQuaternionToRef(LIB.Space.WORLD, this.mesh, this._boneQuat);
                }
                if (this._transformYawPitch) {
                    this._transformYawPitch.multiplyToRef(_tmpMat1, _tmpMat1);
                }
                LIB.Quaternion.FromRotationMatrixToRef(_tmpMat1, _tmpQuat);
                LIB.Quaternion.SlerpToRef(this._boneQuat, _tmpQuat, this.slerpAmount, this._boneQuat);
                this.bone.setRotationQuaternion(this._boneQuat, LIB.Space.WORLD, this.mesh);
                this._slerping = true;
            }
            else {
                if (this._transformYawPitch) {
                ...
                this.bone.setRotationMatrix(_tmpMat1, LIB.Space.WORLD, this.mesh);
                this._slerping = false;
            }
        };
        ...
        BoneLookController._tmpVecs = [LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero(), LIB.Vector3.Zero()];
        BoneLookController._tmpQuat = LIB.Quaternion.Identity();
        BoneLookController._tmpMats = [LIB.Matrix.Identity(), LIB.Matrix.Identity(), LIB.Matrix.Identity(), LIB.Matrix.Identity(), LIB.Matrix.Identity()];
