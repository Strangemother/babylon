    ...
    var CustomProceduralTexture = /** @class */ (function (_super) {
        ...
        CustomProceduralTexture.prototype.loadJson = function (jsonUrl) {
            ...
            var noConfigFile = function () {
                LIB.Tools.Log("No config file found in " + jsonUrl + " trying to use ShadersStore or DOM element");
                try {
                    _this.setFragment(_this._texturePath);
                }
                catch (ex) {
                    LIB.Tools.Error("No json or ShaderStore or DOM element found for CustomProceduralTexture");
                }
            };
            ...
            try {
                ...
                LIB.Tools.Error("CustomProceduralTexture: Error on XHR send request.");
            }
        };
        ...
        CustomProceduralTexture.prototype.updateTextures = function () {
            for (var i = 0; i < this._config.sampler2Ds.length; i++) {
                this.setTexture(this._config.sampler2Ds[i].sample2Dname, new LIB.Texture(this._texturePath + "/" + this._config.sampler2Ds[i].textureRelativeUrl, this.getScene()));
            }
        };
                ...
                for (var j = 0; j < this._config.uniforms.length; j++) {
                    ...
                    switch (uniform.type) {
                        case "float":
                            ...
                            break;
                        case "color3":
                            this.setColor3(uniform.name, new LIB.Color3(uniform.r, uniform.g, uniform.b));
                            break;
                        case "color4":
                            this.setColor4(uniform.name, new LIB.Color4(uniform.r, uniform.g, uniform.b, uniform.a));
                            break;
                        case "vector2":
                            this.setVector2(uniform.name, new LIB.Vector2(uniform.x, uniform.y));
                            break;
                        case "vector3":
                            this.setVector3(uniform.name, new LIB.Vector3(uniform.x, uniform.y, uniform.z));
                            break;
                    }
                }
            }
