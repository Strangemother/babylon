...
(function (LIB) {
    ...
    var getSamplingParameters = function (samplingMode, generateMipMaps, gl) {
        ...
        switch (samplingMode) {
            case LIB.Texture.BILINEAR_SAMPLINGMODE:
                magFilter = gl.LINEAR;
                if (generateMipMaps) {
                ...
                break;
            case LIB.Texture.TRILINEAR_SAMPLINGMODE:
                magFilter = gl.LINEAR;
                if (generateMipMaps) {
                ...
                break;
            case LIB.Texture.NEAREST_SAMPLINGMODE:
                ...
                break;
            case LIB.Texture.NEAREST_NEAREST_MIPNEAREST:
                magFilter = gl.NEAREST;
                if (generateMipMaps) {
                ...
                break;
            case LIB.Texture.NEAREST_LINEAR_MIPNEAREST:
                magFilter = gl.NEAREST;
                if (generateMipMaps) {
                    minFilter = gl.LINEAR_MIPMAP_NEAREST;
                }
                ...
                break;
            case LIB.Texture.NEAREST_LINEAR_MIPLINEAR:
                magFilter = gl.NEAREST;
                if (generateMipMaps) {
                    minFilter = gl.LINEAR_MIPMAP_LINEAR;
                }
                ...
                break;
            case LIB.Texture.NEAREST_LINEAR:
                magFilter = gl.NEAREST;
                minFilter = gl.LINEAR;
                break;
            case LIB.Texture.NEAREST_NEAREST:
                magFilter = gl.NEAREST;
                minFilter = gl.NEAREST;
                break;
            case LIB.Texture.LINEAR_NEAREST_MIPNEAREST:
                magFilter = gl.LINEAR;
                if (generateMipMaps) {
                    minFilter = gl.NEAREST_MIPMAP_NEAREST;
                }
                ...
                break;
            case LIB.Texture.LINEAR_NEAREST_MIPLINEAR:
                magFilter = gl.LINEAR;
                if (generateMipMaps) {
                    minFilter = gl.NEAREST_MIPMAP_LINEAR;
                }
                ...
                break;
            case LIB.Texture.LINEAR_LINEAR:
                magFilter = gl.LINEAR;
                minFilter = gl.LINEAR;
                break;
            case LIB.Texture.LINEAR_NEAREST:
                magFilter = gl.LINEAR;
                minFilter = gl.NEAREST;
                break;
        }
    ...
    var partialLoadImg = function (url, index, loadedImages, scene, onfinish, onErrorCallBack) {
        ...
        img = LIB.Tools.LoadImage(url, onload, onerror, scene ? scene.database : null);
        if (scene) {
    ...
    LIB.InstancingAttributeInfo = InstancingAttributeInfo;
    /**
    ...
    var Engine = /** @class */ (function () {
        ...
        function Engine(canvasOrContext, antialias, options, adaptToDeviceRatio) {
             ...
             */
            this.onResizeObservable = new LIB.Observable();
            /**
             * Observable event triggered each time the canvas loses focus
             */
            this.onCanvasBlurObservable = new LIB.Observable();
            /**
             * Observable event triggered each time the canvas gains focus
             */
            this.onCanvasFocusObservable = new LIB.Observable();
            /**
             * Observable event triggered each time the canvas receives pointerout event
             */
            this.onCanvasPointerOutObservable = new LIB.Observable();
            /**
             * Observable event triggered before each texture is initialized
             */
            this.onBeforeTextureInitObservable = new LIB.Observable();
            //WebVR
             ...
             */
            this.onBeginFrameObservable = new LIB.Observable();
            /**
             * Observable raised when the engine ends the current frame
             */
            this.onEndFrameObservable = new LIB.Observable();
            /**
             * Observable raised when the engine is about to compile a shader
             */
            this.onBeforeShaderCompilationObservable = new LIB.Observable();
            /**
            ...
            // FPS
            this._performanceMonitor = new LIB.PerformanceMonitor();
            this._fps = 60;
            this._deltaTime = 0;
            /**
            ...
            this._onVRFullScreenTriggered = function () {
                if (_this._vrDisplay && _this._vrDisplay.isPresenting) {
                    ...
                    _this._oldSize = new LIB.Size(_this.getRenderWidth(), _this.getRenderHeight());
            ...
            if (canvasOrContext.getContext) {
                ...
                if (!this._doNotHandleContextLost) {
                    this._onContextLost = function (evt) {
                        ...
                        LIB.Tools.Warn("WebGL context lost.");
                    ...
                    this._onContextRestored = function (evt) {
                        ...
                        setTimeout(function () {
                            ...
                            LIB.Tools.Warn("WebGL context successfully restored.");
            ...
            if (options.audioEngine && LIB.AudioEngine && !Engine.audioEngine) {
                Engine.audioEngine = new LIB.AudioEngine();
            }
        ...
        Object.defineProperty(Engine.prototype, "emptyCubeTexture", {
            ...
            get: function () {
                if (!this._emptyCubeTexture) {
                    ...
                    this._emptyCubeTexture = this.createRawCubeTexture(cubeData, 1, Engine.TEXTUREFORMAT_RGBA, Engine.TEXTURETYPE_UNSIGNED_INT, false, false, LIB.Texture.NEAREST_SAMPLINGMODE);
                }
        ...
        Engine.prototype._renderLoop = function () {
            ...
            if (this._activeRenderLoops.length > 0) {
                ...
                this._frameHandler = LIB.Tools.QueueNewFrame(this._bindedRenderFunction, requester);
            }
            else {
                this._renderingQueueLaunched = false;
            }
        ...
        Engine.prototype.runRenderLoop = function (renderFunction) {
            ...
            if (!this._renderingQueueLaunched) {
                ...
                this._frameHandler = LIB.Tools.QueueNewFrame(this._bindedRenderFunction);
            }
        };
        /**
        ...
        Engine.prototype.switchFullscreen = function (requestPointerLock) {
            if (this.isFullscreen) {
                LIB.Tools.ExitFullscreen();
            }
            else {
                this._pointerLockRequested = requestPointerLock;
                if (this._renderingCanvas) {
                    LIB.Tools.RequestFullscreen(this._renderingCanvas);
                }
            }
        };
        /**
        ...
        Engine.prototype.initWebVRAsync = function () {
            ...
            if (!this._onVrDisplayConnect) {
                ...
                this._onVrDisplayDisconnect = function () {
                    ...
                    _this._frameHandler = LIB.Tools.QueueNewFrame(_this._bindedRenderFunction);
                    notifyObservers();
                };
        ...
        Engine.prototype.createEffect = function (baseName, attributesNamesOrOptions, uniformsNamesOrEngine, samplers, defines, fallbacks, onCompiled, onError, indexParameters) {
            ...
            var effect = new LIB.Effect(baseName, attributesNamesOrOptions, uniformsNamesOrEngine, samplers, this, defines, fallbacks, onCompiled, onError, indexParameters);
            effect._key = name;
        ...
        Engine.prototype.createTexture = function (urlArg, noMipmap, invertY, scene, samplingMode, onLoad, onError, buffer, fallback, format) {
            ...
            var onerror = function (message, exception) {
                ...
                else if (LIB.Tools.UseFallbackTexture) {
                    _this.createTexture(LIB.Tools.fallbackTexture, noMipmap, invertY, scene, samplingMode, null, onError, buffer, texture);
                }
                if (onError) {
            ...
            if (isKTX || isTGA || isDDS) {
                ...
                var onload = function (img) {
                    ...
                    _this._prepareWebGLTexture(texture, scene, img.width, img.height, invertY, noMipmap, false, function (potWidth, potHeight, continuationCallback) {
                        ...
                        var source = new LIB.InternalTexture(_this, LIB.InternalTexture.DATASOURCE_TEMP);
                ...
                if (!fromData || isBase64)
                    if (buffer instanceof HTMLImageElement) {
                        ...
                        LIB.Tools.LoadImage(url, onload, onerror, scene ? scene.database : null);
                    }
                else if (buffer instanceof Array || typeof buffer === "string" || buffer instanceof ArrayBuffer)
                    LIB.Tools.LoadImage(buffer, onload, onerror, scene ? scene.database : null);
                else
                    onload(buffer);
            }
            return texture;
        ...
        Engine.prototype._rescaleTexture = function (source, destination, scene, internalFormat, onComplete) {
            ...
            var rtt = this.createRenderTargetTexture({
                ...
                samplingMode: LIB.Texture.BILINEAR_SAMPLINGMODE,
                generateDepthBuffer: false,
                generateStencilBuffer: false
            });
            if (!this._rescalePostProcess) {
                this._rescalePostProcess = new LIB.PassPostProcess("rescale", 1, null, LIB.Texture.BILINEAR_SAMPLINGMODE, this, false, Engine.TEXTURETYPE_UNSIGNED_INT);
            }
        ...
        Engine.prototype.createRawTexture = function (data, width, height, format, generateMipMaps, invertY, samplingMode, compression, type) {
            if (compression === void 0) { compression = null; }
            if (type === void 0) { type = Engine.TEXTURETYPE_UNSIGNED_INT; }
            var texture = new LIB.InternalTexture(this, LIB.InternalTexture.DATASOURCE_RAW);
        ...
        Engine.prototype.createDynamicTexture = function (width, height, generateMipMaps, samplingMode) {
            ...
            texture.baseHeight = height;
            if (generateMipMaps) {
                width = this.needPOTTextures ? LIB.Tools.GetExponentOfTwo(width, this._caps.maxTextureSize) : width;
                height = this.needPOTTextures ? LIB.Tools.GetExponentOfTwo(height, this._caps.maxTextureSize) : height;
            }
            //  this.resetTextureCache();
            texture.width = width;
            texture.height = height;
        ...
        Engine.prototype.createRenderTargetTexture = function (size, options) {
            ...
            if (fullOptions.type === Engine.TEXTURETYPE_FLOAT && !this._caps.textureFloat) {
                fullOptions.type = Engine.TEXTURETYPE_UNSIGNED_INT;
                LIB.Tools.Warn("Float textures are not supported. Render target forced to TEXTURETYPE_UNSIGNED_BYTE type");
            }
        ...
        Engine.prototype.createMultipleRenderTarget = function (size, options) {
            ...
            for (var i = 0; i < textureCount; i++) {
                ...
                if (type === Engine.TEXTURETYPE_FLOAT && !this._caps.textureFloatLinearFiltering) {
                    // if floating point linear (gl.FLOAT) then force to NEAREST_SAMPLINGMODE
                    samplingMode = LIB.Texture.NEAREST_SAMPLINGMODE;
                }
                ...
                var texture = new LIB.InternalTexture(this, LIB.InternalTexture.DATASOURCE_MULTIRENDERTARGET);
            ...
            if (generateDepthTexture && this._caps.depthTextureExtension) {
                // Depth texture
                var depthTexture = new LIB.InternalTexture(this, LIB.InternalTexture.DATASOURCE_MULTIRENDERTARGET);
        ...
        Engine.prototype.createPrefilteredCubeTexture = function (rootUrl, scene, scale, offset, onLoad, onError, format, forcedExtension) {
            ...
            var callback = function (loadData) {
                ...
                for (var i = 0; i < mipSlices; i++) {
                    ...
                    var maxLODIndex = LIB.Scalar.Log2(width) * scale + offset; // roughness = 1
                    ...
                    var glTextureFromLod = new LIB.InternalTexture(_this, LIB.InternalTexture.DATASOURCE_TEMP);
                    ...
                    var lodTexture = new LIB.BaseTexture(scene);
        ...
        Engine.prototype.createCubeTexture = function (rootUrl, scene, files, noMipmap, onLoad, onError, format, forcedExtension, createPolynomials) {
            ...
            if (isKTX) {
                ...
                cascadeLoadImgs(rootUrl, scene, function (imgs) {
                    var width = _this.needPOTTextures ? LIB.Tools.GetExponentOfTwo(imgs[0].width, _this._caps.maxCubemapTextureSize) : imgs[0].width;
                    var height = width;
        ...
        Engine.prototype.updateRawCubeTexture = function (texture, data, format, type, invertY, compression, level) {
            ...
            var isPot = !this.needPOTTextures || (LIB.Tools.IsExponentOfTwo(texture.width) && LIB.Tools.IsExponentOfTwo(texture.height));
        ...
        Engine.prototype.createRawCubeTexture = function (data, size, format, type, generateMipMaps, invertY, samplingMode, compression) {
            if (compression === void 0) { compression = null; }
            var gl = this._gl;
            var texture = new LIB.InternalTexture(this, LIB.InternalTexture.DATASOURCE_CUBERAW);
            texture.isCube = true;
            texture.generateMipMaps = generateMipMaps;
            texture.format = format;
            texture.type = type;
        ...
        Engine.prototype._prepareWebGLTexture = function (texture, scene, width, height, invertY, noMipmap, isCompressed, processFunction, samplingMode) {
            var _this = this;
            if (samplingMode === void 0) { samplingMode = LIB.Texture.TRILINEAR_SAMPLINGMODE; }
            var potWidth = this.needPOTTextures ? LIB.Tools.GetExponentOfTwo(width, this.getCaps().maxTextureSize) : width;
            var potHeight = this.needPOTTextures ? LIB.Tools.GetExponentOfTwo(height, this.getCaps().maxTextureSize) : height;
            var gl = this._gl;
            if (!gl) {
                return;
            }
        ...
        Engine.prototype._setAnisotropicLevel = function (target, texture) {
            ...
            if (internalTexture.samplingMode !== LIB.Texture.LINEAR_LINEAR_MIPNEAREST
                && internalTexture.samplingMode !== LIB.Texture.LINEAR_LINEAR_MIPLINEAR
                && internalTexture.samplingMode !== LIB.Texture.LINEAR_LINEAR) {
