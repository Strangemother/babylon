...
(function (LIB) {
    ...
    var EdgesRenderer = /** @class */ (function () {
        ...
        EdgesRenderer.prototype._prepareRessources = function () {
            ...
            this._lineShader = new LIB.ShaderMaterial("lineShader", this._source.getScene(), "line", {
        ...
        EdgesRenderer.prototype._rebuild = function () {
            var buffer = this._buffers[LIB.VertexBuffer.PositionKind];
            if (buffer) {
                buffer._rebuild();
            }
            buffer = this._buffers[LIB.VertexBuffer.NormalKind];
            if (buffer) {
                buffer._rebuild();
            }
        ...
        EdgesRenderer.prototype.dispose = function () {
            ...
            if (buffer) {
                buffer.dispose();
                this._buffers[LIB.VertexBuffer.PositionKind] = null;
            }
            buffer = this._buffers[LIB.VertexBuffer.NormalKind];
            if (buffer) {
                buffer.dispose();
                this._buffers[LIB.VertexBuffer.NormalKind] = null;
            }
        ...
        EdgesRenderer.prototype._checkEdge = function (faceIndex, edge, faceNormals, p0, p1) {
            ...
            if (edge === undefined) {
                ...
                var dotProduct = LIB.Vector3.Dot(faceNormals[faceIndex], faceNormals[edge]);
        ...
        EdgesRenderer.prototype._generateEdgesLines = function () {
            var positions = this._source.getVerticesData(LIB.VertexBuffer.PositionKind);
            ...
            for (index = 0; index < indices.length; index += 3) {
                ...
                faceAdjacencies.p0 = new LIB.Vector3(positions[p0Index * 3], positions[p0Index * 3 + 1], positions[p0Index * 3 + 2]);
                faceAdjacencies.p1 = new LIB.Vector3(positions[p1Index * 3], positions[p1Index * 3 + 1], positions[p1Index * 3 + 2]);
                faceAdjacencies.p2 = new LIB.Vector3(positions[p2Index * 3], positions[p2Index * 3 + 1], positions[p2Index * 3 + 2]);
                var faceNormal = LIB.Vector3.Cross(faceAdjacencies.p1.subtract(faceAdjacencies.p0), faceAdjacencies.p2.subtract(faceAdjacencies.p1));
            ...
            this._buffers[LIB.VertexBuffer.PositionKind] = new LIB.VertexBuffer(engine, this._linesPositions, LIB.VertexBuffer.PositionKind, false);
            this._buffers[LIB.VertexBuffer.NormalKind] = new LIB.VertexBuffer(engine, this._linesNormals, LIB.VertexBuffer.NormalKind, false, false, 4);
            this._ib = engine.createIndexBuffer(this._linesIndices);
        ...
        EdgesRenderer.prototype.render = function () {
            ...
            if (scene.activeCamera.mode === LIB.Camera.ORTHOGRAPHIC_CAMERA) {
            ...
            // Draw order
            engine.drawElementsType(LIB.Material.TriangleFillMode, 0, this._indicesCount);
