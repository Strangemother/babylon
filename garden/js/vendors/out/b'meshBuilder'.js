...
(function (LIB) {
    ...
    var MeshBuilder = /** @class */ (function () {
        ...
        MeshBuilder.updateSideOrientation = function (orientation) {
            if (orientation == LIB.Mesh.DOUBLESIDE) {
                return LIB.Mesh.DOUBLESIDE;
            }
            if (orientation === undefined || orientation === null) {
                return LIB.Mesh.FRONTSIDE;
            }
            return orientation;
        };
        /**
         ...
         * * Please read this tutorial : http://doc.LIBjs.com/tutorials/CreateCube_Per_Face_Textures_And_Colors
         ...
         * * If you create a double-sided mesh, you can choose what parts of the texture image to crop and stick respectively on the front and the back sides with the parameters `frontUVs` and `backUVs` (Vector4). Detail here : http://doc.LIBjs.com/LIB101/discover_basic_elements#side-orientation
         * * The mesh can be set to updatable with the boolean parameter `updatable` (default false) if its internal geometry is supposed to change once created
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#box
         * @param name defines the name of the mesh
         * @param options defines the options used to create the mesh
         * @param scene defines the hosting scene
         * @returns the box mesh
         */
        MeshBuilder.CreateCube = function (name, options, scene) {
            if (scene === void 0) { scene = null; }
            var box = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            box._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateCube(options);
         ...
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#sphere
         */
        MeshBuilder.CreateSphere = function (name, options, scene) {
            var sphere = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            sphere._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateSphere(options);
         ...
         * @see http://doc.LIBjs.com/how_to/set_shapes#disc-or-regular-polygon
         */
        MeshBuilder.CreateDisc = function (name, options, scene) {
            if (scene === void 0) { scene = null; }
            var disc = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            disc._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateDisc(options);
         ...
         * @see http://doc.LIBjs.com/how_to/polyhedra_shapes#icosphere
         */
        MeshBuilder.CreateIcoSphere = function (name, options, scene) {
            var sphere = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            sphere._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateIcoSphere(options);
            vertexData.applyToMesh(sphere, options.updatable);
            return sphere;
        };
        ;
         ...
         * @see http://doc.LIBjs.com/tutorials/Ribbon_Tutorial
         * @see http://doc.LIBjs.com/tutorials/Parametric_Shapes
         */
        MeshBuilder.CreateRibbon = function (name, options, scene) {
            ...
            if (instance) { // existing ribbon instance update
                ...
                LIB.Vector3.FromFloatsToRef(Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, LIB.Tmp.Vector3[0]); // minimum
                LIB.Vector3.FromFloatsToRef(-Number.MAX_VALUE, -Number.MAX_VALUE, -Number.MAX_VALUE, LIB.Tmp.Vector3[1]);
                var positionFunction = function (positions) {
                    var minlg = pathArray[0].length;
                    var i = 0;
                    var ns = (instance._originalBuilderSideOrientation === LIB.Mesh.DOUBLESIDE) ? 2 : 1;
                    for (var si = 1; si <= ns; si++) {
                        for (var p = 0; p < pathArray.length; p++) {
                            ...
                            while (j < minlg) {
                                ...
                                if (path[j].x < LIB.Tmp.Vector3[0].x) {
                                    LIB.Tmp.Vector3[0].x = path[j].x;
                                }
                                if (path[j].x > LIB.Tmp.Vector3[1].x) {
                                    LIB.Tmp.Vector3[1].x = path[j].x;
                                }
                                if (path[j].y < LIB.Tmp.Vector3[0].y) {
                                    LIB.Tmp.Vector3[0].y = path[j].y;
                                }
                                if (path[j].y > LIB.Tmp.Vector3[1].y) {
                                    LIB.Tmp.Vector3[1].y = path[j].y;
                                }
                                if (path[j].z < LIB.Tmp.Vector3[0].z) {
                                    LIB.Tmp.Vector3[0].z = path[j].z;
                                }
                                if (path[j].z > LIB.Tmp.Vector3[1].z) {
                                    LIB.Tmp.Vector3[1].z = path[j].z;
                                }
                                j++;
                                i += 3;
                            }
                ...
                var positions = instance.getVerticesData(LIB.VertexBuffer.PositionKind);
                positionFunction(positions);
                instance._boundingInfo = new LIB.BoundingInfo(LIB.Tmp.Vector3[0], LIB.Tmp.Vector3[1]);
                instance._boundingInfo.update(instance._worldMatrix);
                instance.updateVerticesData(LIB.VertexBuffer.PositionKind, positions, false, false);
                if (options.colors) {
                    var colors = instance.getVerticesData(LIB.VertexBuffer.ColorKind);
                    ...
                    instance.updateVerticesData(LIB.VertexBuffer.ColorKind, colors, false, false);
                }
                if (options.uvs) {
                    var uvs = instance.getVerticesData(LIB.VertexBuffer.UVKind);
                    for (var i = 0; i < options.uvs.length; i++) {
                        uvs[i * 2] = options.uvs[i].x;
                        uvs[i * 2 + 1] = options.uvs[i].y;
                    }
                    instance.updateVerticesData(LIB.VertexBuffer.UVKind, uvs, false, false);
                }
                if (!instance.areNormalsFrozen || instance.isFacetDataEnabled) {
                    var indices = instance.getIndices();
                    var normals = instance.getVerticesData(LIB.VertexBuffer.NormalKind);
                    var params = instance.isFacetDataEnabled ? instance.getFacetDataParameters() : null;
                    LIB.VertexData.ComputeNormals(positions, indices, normals, params);
                    if (instance._closePath) {
                        var indexFirst = 0;
                        var indexLast = 0;
                        for (var p = 0; p < pathArray.length; p++) {
                    ...
                    if (!(instance.areNormalsFrozen)) {
                        instance.updateVerticesData(LIB.VertexBuffer.NormalKind, normals, false, false);
                    }
                }
                return instance;
            }
            else { // new ribbon creation
                var ribbon = new LIB.Mesh(name, scene);
                ribbon._originalBuilderSideOrientation = sideOrientation;
                var vertexData = LIB.VertexData.CreateRibbon(options);
                if (closePath) {
         ...
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#cylinder-or-cone
         */
        MeshBuilder.CreateCylinder = function (name, options, scene) {
            var cylinder = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            cylinder._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateCylinder(options);
         ...
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#torus
         */
        MeshBuilder.CreateTorus = function (name, options, scene) {
            var torus = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            torus._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateTorus(options);
         ...
         * @see  http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#torus-knot
         */
        MeshBuilder.CreateTorusKnot = function (name, options, scene) {
            var torusKnot = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            torusKnot._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreateTorusKnot(options);
         ...
         * * Updating a simple Line mesh, you just need to update every line in the `lines` array : http://doc.LIBjs.com/tutorials/How_to_dynamically_morph_a_mesh#lines-and-dashedlines
         * * When updating an instance, remember that only line point positions can change, not the number of points, neither the number of lines
         * * The mesh can be set to updatable with the boolean parameter `updatable` (default false) if its internal geometry is supposed to change once created
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#line-system
        ...
        MeshBuilder.CreateLineSystem = function (name, options, scene) {
            ...
            if (instance) { // lines update
                ...
                var lineColors;
                if (colors) {
                    vertexColor = instance.getVerticesData(LIB.VertexBuffer.ColorKind);
                }
                var i = 0;
                var c = 0;
                ...
                instance.updateVerticesData(LIB.VertexBuffer.PositionKind, positions, false, false);
                if (colors && vertexColor) {
                    instance.updateVerticesData(LIB.VertexBuffer.ColorKind, vertexColor, false, false);
                }
                return instance;
            }
            // line system creation
            var useVertexColor = (colors) ? true : false;
            var lineSystem = new LIB.LinesMesh(name, scene, null, undefined, undefined, useVertexColor, options.useVertexAlpha);
            var vertexData = LIB.VertexData.CreateLineSystem(options);
         ...
         * * The mesh can be set to updatable with the boolean parameter `updatable` (default false) if its internal geometry is supposed to change once created
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#lines
         * @param name defines the name of the new line system
         * @param options defines the options used to create the line system
         * @param scene defines the hosting scene
         ...
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#dashed-lines
         */
        MeshBuilder.CreateDashedLines = function (name, options, scene) {
            ...
            var dashedLines = new LIB.LinesMesh(name, scene);
            var vertexData = LIB.VertexData.CreateDashedLines(options);
         ...
         * @see http://doc.LIBjs.com/tutorials/Parametric_Shapes
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#extruded-shapes
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#extruded-shapes
         */
        MeshBuilder.ExtrudeShape = function (name, options, scene) {
            ...
            var cap = (options.cap === 0) ? 0 : options.cap || LIB.Mesh.NO_CAP;
            var updatable = options.updatable;
            var sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
         ...
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#custom-extruded-shapes
         * @see http://doc.LIBjs.com/tutorials/Parametric_Shapes
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#extruded-shapes
         */
         ...
         * @see http://doc.LIBjs.com/how_to/parametric_shapes#lathe
         */
        MeshBuilder.CreateLathe = function (name, options, scene) {
            ...
            var updatable = options.updatable;
            var sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            var cap = options.cap || LIB.Mesh.NO_CAP;
            var pi2 = Math.PI * 2;
            var paths = new Array();
            var invertUV = options.invertUV || false;
            var i = 0;
            ...
            for (i = 0; i <= tessellation; i++) {
                var path = [];
                if (cap == LIB.Mesh.CAP_START || cap == LIB.Mesh.CAP_ALL) {
                    path.push(new LIB.Vector3(0, shape[0].y, 0));
                    path.push(new LIB.Vector3(Math.cos(i * step) * shape[0].x * radius, shape[0].y, Math.sin(i * step) * shape[0].x * radius));
                }
                for (p = 0; p < shape.length; p++) {
                    rotated = new LIB.Vector3(Math.cos(i * step) * shape[p].x * radius, shape[p].y, Math.sin(i * step) * shape[p].x * radius);
                    path.push(rotated);
                }
                if (cap == LIB.Mesh.CAP_END || cap == LIB.Mesh.CAP_ALL) {
                    path.push(new LIB.Vector3(Math.cos(i * step) * shape[shape.length - 1].x * radius, shape[shape.length - 1].y, Math.sin(i * step) * shape[shape.length - 1].x * radius));
                    path.push(new LIB.Vector3(0, shape[shape.length - 1].y, 0));
                }
                paths.push(path);
            }
            // lathe ribbon
         ...
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#plane
         */
        MeshBuilder.CreatePlane = function (name, options, scene) {
            var plane = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            plane._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreatePlane(options);
            vertexData.applyToMesh(plane, options.updatable);
            if (options.sourcePlane) {
                plane.translate(options.sourcePlane.normal, options.sourcePlane.d);
                var product = Math.acos(LIB.Vector3.Dot(options.sourcePlane.normal, LIB.Axis.Z));
                var vectorProduct = LIB.Vector3.Cross(LIB.Axis.Z, options.sourcePlane.normal);
        ...
        MeshBuilder.CreateGround = function (name, options, scene) {
            var ground = new LIB.GroundMesh(name, scene);
            ...
            var vertexData = LIB.VertexData.CreateGround(options);
         ...
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#tiled-ground
         */
        MeshBuilder.CreateTiledGround = function (name, options, scene) {
            var tiledGround = new LIB.Mesh(name, scene);
            var vertexData = LIB.VertexData.CreateTiledGround(options);
         ...
         * @param scene defines the hosting scene
         * @returns the ground mesh
         * @see http://doc.LIBjs.com/LIB101/height_map
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#ground-from-a-height-map
         */
        MeshBuilder.CreateGroundFromHeightMap = function (name, url, options, scene) {
            ...
            var onload = function (img) {
                ...
                var vertexData = LIB.VertexData.CreateGroundFromHeightMap({
        ...
        MeshBuilder.CreatePolygon = function (name, options, scene) {
            ...
            for (var i = 0; i < shape.length; i++) {
                contours[i] = new LIB.Vector2(shape[i].x, shape[i].z);
            }
            var epsilon = 0.00000001;
            if (contours[0].equalsWithEpsilon(contours[contours.length - 1], epsilon)) {
                contours.pop();
            }
            var polygonTriangulation = new LIB.PolygonMeshBuilder(name, contours, scene);
            for (var hNb = 0; hNb < holes.length; hNb++) {
                hole = [];
                for (var hPoint = 0; hPoint < holes[hNb].length; hPoint++) {
                    hole.push(new LIB.Vector2(holes[hNb][hPoint].x, holes[hNb][hPoint].z));
                }
                polygonTriangulation.addHole(hole);
            }
            var polygon = polygonTriangulation.build(options.updatable, depth);
            polygon._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreatePolygon(polygon, options.sideOrientation, options.faceUV, options.faceColors, options.frontUVs, options.backUVs);
         ...
         * @see http://doc.LIBjs.com/tutorials/CreateCube_Per_Face_Textures_And_Colors
         * @param name defines the name of the mesh
         * @param options defines the options used to create the mesh
         * @param scene defines the hosting scene
         * @returns the polygon mesh
         ...
         * @see http://doc.LIBjs.com/tutorials/Parametric_Shapes
         * @see http://doc.LIBjs.com/tutorials/Mesh_CreateXXX_Methods_With_Options_Parameter#tube
         */
        MeshBuilder.CreateTube = function (name, options, scene) {
            ...
            var tubePathArray = function (path, path3D, circlePaths, radius, tessellation, radiusFunction, cap, arc) {
                ...
                var normal;
                var rotated;
                var rotationMatrix = LIB.Tmp.Matrix[0];
                var index = (cap === LIB.Mesh._NO_CAP || cap === LIB.Mesh.CAP_END) ? 0 : 2;
                for (var i = 0; i < path.length; i++) {
                    rad = radiusFunctionFinal(i, distances[i]); // current radius
                    circlePath = Array(); // current circle array
                    normal = normals[i]; // current normal
                    for (var t = 0; t < tessellation; t++) {
                        LIB.Matrix.RotationAxisToRef(tangents[i], step * t, rotationMatrix);
                        rotated = circlePath[t] ? circlePath[t] : LIB.Vector3.Zero();
                        LIB.Vector3.TransformCoordinatesToRef(normal, rotationMatrix, rotated);
                ...
                switch (cap) {
                    case LIB.Mesh.NO_CAP:
                        break;
                    case LIB.Mesh.CAP_START:
                        circlePaths[0] = capPath(tessellation, 0);
                        circlePaths[1] = circlePaths[2].slice(0);
                        break;
                    case LIB.Mesh.CAP_END:
                        circlePaths[index] = circlePaths[index - 1].slice(0);
                        circlePaths[index + 1] = capPath(tessellation, path.length - 1);
                        break;
                    case LIB.Mesh.CAP_ALL:
                        circlePaths[0] = capPath(tessellation, 0);
                        circlePaths[1] = circlePaths[2].slice(0);
                        circlePaths[index] = circlePaths[index - 1].slice(0);
                        circlePaths[index + 1] = capPath(tessellation, path.length - 1);
         ...
         * @see http://doc.LIBjs.com/how_to/polyhedra_shapes
         */
        MeshBuilder.CreatePolyhedron = function (name, options, scene) {
            var polyhedron = new LIB.Mesh(name, scene);
            options.sideOrientation = MeshBuilder.updateSideOrientation(options.sideOrientation);
            polyhedron._originalBuilderSideOrientation = options.sideOrientation;
            var vertexData = LIB.VertexData.CreatePolyhedron(options);
         ...
         * @see http://doc.LIBjs.com/how_to/decals
         */
        MeshBuilder.CreateDecal = function (name, sourceMesh, options) {
            var indices = sourceMesh.getIndices();
            var positions = sourceMesh.getVerticesData(LIB.VertexBuffer.PositionKind);
            var normals = sourceMesh.getVerticesData(LIB.VertexBuffer.NormalKind);
            var position = options.position || LIB.Vector3.Zero();
            var normal = options.normal || LIB.Vector3.Up();
            var size = options.size || LIB.Vector3.One();
            var angle = options.angle || 0;
            // Getting correct rotation
            if (!normal) {
                var target = new LIB.Vector3(0, 0, 1);
                var camera = sourceMesh.getScene().activeCamera;
                var cameraWorldTarget = LIB.Vector3.TransformCoordinates(target, camera.getWorldMatrix());
            ...
            // Matrix
            var decalWorldMatrix = LIB.Matrix.RotationYawPitchRoll(yaw, pitch, angle).multiply(LIB.Matrix.Translation(position.x, position.y, position.z));
            var inverseDecalWorldMatrix = LIB.Matrix.Invert(decalWorldMatrix);
            ...
            var extractDecalVector3 = function (indexId) {
                var result = new LIB.PositionNormalVertex();
                if (!indices || !positions || !normals) {
                    return result;
                }
                var vertexId = indices[indexId];
                result.position = new LIB.Vector3(positions[vertexId * 3], positions[vertexId * 3 + 1], positions[vertexId * 3 + 2]);
                // Send vector to decal local world
                result.position = LIB.Vector3.TransformCoordinates(result.position, transformMatrix);
                // Get normal
                result.normal = new LIB.Vector3(normals[vertexId * 3], normals[vertexId * 3 + 1], normals[vertexId * 3 + 2]);
                result.normal = LIB.Vector3.TransformNormal(result.normal, transformMatrix);
                return result;
            ...
            var clip = function (vertices, axis) {
                ...
                var clipSize = 0.5 * Math.abs(LIB.Vector3.Dot(size, axis));
                var clipVertices = function (v0, v1) {
                    var clipFactor = LIB.Vector3.GetClipFactor(v0.position, v1.position, axis, clipSize);
                    return new LIB.PositionNormalVertex(LIB.Vector3.Lerp(v0.position, v1.position, clipFactor), LIB.Vector3.Lerp(v0.normal, v1.normal, clipFactor));
                };
                ...
                for (var index = 0; index < vertices.length; index += 3) {
                    ...
                    var nV3 = null;
                    var nV4 = null;
                    var d1 = LIB.Vector3.Dot(vertices[index].position, axis) - clipSize;
                    var d2 = LIB.Vector3.Dot(vertices[index + 1].position, axis) - clipSize;
                    var d3 = LIB.Vector3.Dot(vertices[index + 2].position, axis) - clipSize;
                    v1Out = d1 > 0;
                    v2Out = d2 > 0;
                    v3Out = d3 > 0;
            ...
            for (var index = 0; index < indices.length; index += 3) {
                ...
                // Clip
                faceVertices = clip(faceVertices, new LIB.Vector3(1, 0, 0));
                faceVertices = clip(faceVertices, new LIB.Vector3(-1, 0, 0));
                faceVertices = clip(faceVertices, new LIB.Vector3(0, 1, 0));
                faceVertices = clip(faceVertices, new LIB.Vector3(0, -1, 0));
                faceVertices = clip(faceVertices, new LIB.Vector3(0, 0, 1));
                faceVertices = clip(faceVertices, new LIB.Vector3(0, 0, -1));
            ...
            // Return mesh
            var decal = new LIB.Mesh(name, sourceMesh.getScene());
            vertexData.applyToMesh(decal);
            decal.position = position.clone();
            decal.rotation = new LIB.Vector3(pitch, yaw, angle);
            return decal;
        };
        // Privates
        MeshBuilder._ExtrudeShapeGeneric = function (name, shape, curve, scale, rotation, scaleFunction, rotateFunction, rbCA, rbCP, cap, custom, scene, updtbl, side, instance, invertUV, frontUVs, backUVs) {
            ...
            var extrusionPathArray = function (shape, curve, path3D, shapePaths, scale, rotation, scaleFunction, rotateFunction, cap, custom) {
                ...
                var index = (cap === LIB.Mesh.NO_CAP || cap === LIB.Mesh.CAP_END) ? 0 : 2;
                var rotationMatrix = LIB.Tmp.Matrix[0];
                for (var i = 0; i < curve.length; i++) {
                    ...
                    for (var p = 0; p < shape.length; p++) {
                        LIB.Matrix.RotationAxisToRef(tangents[i], angle, rotationMatrix);
                        var planed = ((tangents[i].scale(shape[p].z)).add(normals[i].scale(shape[p].x)).add(binormals[i].scale(shape[p].y)));
                        var rotated = shapePath[p] ? shapePath[p] : LIB.Vector3.Zero();
                        LIB.Vector3.TransformCoordinatesToRef(planed, rotationMatrix, rotated);
            ...
            if (instance) { // instance update
                ...
                instance = LIB.Mesh.CreateRibbon("", pathArray, false, false, 0, scene || undefined, false, 0, instance);
                return instance;
            }
            // extruded shape creation
            path3D = new LIB.Path3D(curve);
    ...
    LIB.MeshBuilder = MeshBuilder;
})(LIB || (LIB = {}));

//# sourceMappingURL=LIB.meshBuilder.js.map
//# sourceMappingURL=LIB.meshBuilder.js.map
