...
;TEMPLATER = (function(global) {
    ...
    var template = global.template = function template(str, data, c, def){
        let templateFunc = function(v){return v}
        if(window['doT'] != undefined) {
            templateFunc = doT.template(str, c, def)
        };

        return templateFunc(data)
    }

    var templateMap = global.templateMap = {
        root: 'loadPath'
    }

    global.proxy = function proxy(parent){
        return new Proxy(parent, {
            get: function(t, key) {
                if(key in templateMap) {
                    return t._config[templateMap[key]];
                };
                ...
                console.warn('Template key does not exist:', key);
            }
        });
    }
...
var main = function(){
    /* Start the interface (from bottom of file) */
    api = global.basket;
    global.assetLoader = new AssetLoader();
    global.template = TEMPLATER.template
}
...
class AssetLoader {
    ...
    constructor(config){
        ...
        this.templateProxy = TEMPLATER.proxy(this);

        this.config(config);
    }

    unique(){
        /* Return a unique value for the cache bump.*/
    ...
    defaults(){
        /* Return default objects for configuration */
        var uni = this.unique();
        var res = {
            skipCache: false
            , expire: undefined
    ...
    config(conf){
        ...
        if(conf !== undefined) {
            this._config = Object.assign(this.defaults(), this._config, conf);
        }
    ...
    remove(name) {
        /* Remove an item from the cache by name*/
        return this;
    }

    assets(assets) {
        /* provide a list of assets for 'load' */
        if(assets == undefined) {
            return this._assets;
        };
    ...
    require(assets, append=false){
        var res = [];

        var f = this._requireItem;

        if(this._disabled) {
            console.info('Skip cache due to disabled() flag.')
            f = this._load
        };
        ...
        if(Array.isArray(assets)) {
            ...
            for (var i = assets.length - 1; i >= 0; i--) {
                res.push(f.call(this, assets[i]));
            }
        } else {
            res.push(f.call(this, assets));
        };

        if(append == true && this._requireCache != undefined) {
            res = this._requireCache.concat(res);
        };

        this._requireCache = res;

        return res;
    }

    append(assets) {
        /* load assets with a dialect to associate for _after_ an initial _run_.*/
        return this.load(assets, true)
    }

    _load(assetObject) {
        let d = {};
        return [ this.getTemplateUrl(assetObject) ];
    }

    getTemplateUrl(object) {
        return template(object.url, this.templateProxy)
    }

    _requireItem(assetObject) {
        assetObject.url = this.getTemplateUrl(assetObject)
        return api.require(assetObject);
    }


    load(assets) {
        /* Provide a list of assets to load, assets are loaded if required
        and cached.
        Next load will be cached.*/
        assets = assets == undefined? this.assets(): assets;

        this._loading = this.require( this.buildAssets(assets) );
        if(this._disabled) {
            return ljs.load.apply(ljs, this._requireCache)
        };

        return this._loading
    }

    buildAssets(assets, config) {
        var res = []
        var conf = this.config()

        if(config !== undefined) {
            conf = this.config(config).config();
        };

        for (var i = assets.length - 1; i >= 0; i--) {
            var asset = this.buildAsset(assets[i], conf);
            res.push(asset);
        };
    ...
    buildAsset(asset, conf) {
        /* Provide an asset, returning an object for require with
        configurations applied. */

        var c = { url: asset };
        var res = this.defaults();

        if( typeof(asset) != 'string') {
            c = asset;
        };

        res = Object.assign(res, conf, c);
        return res;
    }
}
